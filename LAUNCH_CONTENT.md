...
All ready-to-post content for launch day.
...
- Security profiles (URL allowlists, rate limiting)
- Battle-tested: 300+ test steps across production apps
...
Set `BROWSER_HEADLESS=false` to watch the browser in real-time — it's surprisingly satisfying to see Claude clicking around your app.

**Security:** Three profiles — `local-dev` (all HTTP/HTTPS, 60 req/min), `restricted` (localhost only, no JS exec), `open` (everything).

**Real-world numbers:** 300+ test steps, 97% pass rate across production apps.

MIT license. TypeScript, Playwright, MCP SDK.

GitHub: https://github.com/DebugBase/glance
Website: https://glance.debugbase.io
...
The core idea: Claude shouldn't have to guess what your web app looks like. Screenshots are returned as base64 images inline, and accessibility snapshots give the full page structure as text. Claude sees the actual rendered output.
...
Security has three profiles controlling URL access, JS execution, and rate limits. The `restricted` profile limits to localhost with JS execution disabled.
...
3. Getting Started (one-liner install)
4. The 30 Tools, Explained (with code examples)
5. Test Automation That Actually Works (JSON scenario examples)
6. Visual Regression Testing (pixelmatch)
7. Security Model (Three profiles)
8. Real-World Use Cases (300+ steps, 97% pass rate)
9. How It Compares to Playwright MCP
10. Contributing & What's Next

### Key code blocks to include:
1. `.mcp.json` config
2. Login flow test scenario JSON
3. Assertion types table
4. Architecture diagram
5. Security profiles table
